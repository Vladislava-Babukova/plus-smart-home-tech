...
public class AggregationStarter {
    ...
    public void start() {
        CompletableFuture.runAsync(() -> {
            Thread.currentThread().setName("AggregationThread");
    ...
    public void shutdown() {
        log.info("Получен сигнал завершения работы. Прерываю работу консьюмера.");
        consumer.wakeup();
    }
        ...
        try {
            ...
            snapshot.ifPresent(s -> {
                ProducerRecord<String, SensorsSnapshotAvro> producerRecord =
                        new ProducerRecord<>(producerConfig.getTopic(), s.getHubId(), s);
